...
import { getPreVisitBrief } from '../../services/ai/aiService';
...
const PreVisitBriefPanel = ({ appointmentId }: { appointmentId: string }) => {
  ...
  const prepare = async () => { if (loading) return; setLoading(true); setError(''); try { setBrief(await getPreVisitBrief(appointmentId)); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'Hasta özeti hazırlanamadı.'); } finally { setLoading(false); } };
